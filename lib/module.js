...
var InventoryModule = (function (_super) {
    ...
    InventoryModule.prototype.__getItems = function (connection, params, callback) {
        var whereCondition = params.selectCondition ? "AND " + params.selectCondition : "";
        connection.query("SELECT ?? FROM item WHERE deleted IS NULL " + whereCondition, [Item.COLUMNS_DB], function (err, rows) {
    ...
    InventoryModule.prototype.__deleteItem = function (connection, params, callback) {
        connection.query("UPDATE item SET deleted=1 WHERE id = ?", [params.id], function (err) {
            callback(err && new DatabaseError(err));
        });
    };
